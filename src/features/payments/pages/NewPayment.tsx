...
export default function NewPayment() {
  ...
  const host = users.find((u) => u.id === selectedHostId);
  ...
  if (step === 'confirm') {
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-8 animate-in slide-in-from-right-8 duration-500 pb-20">
        <button onClick={() => setStep('select')} className="flex items-center gap-2 text-sm text-zinc-400 hover:text-white transition-colors cursor-pointer">
          ...
          <div className="pt-6 border-t border-zinc-800">
            <div className="flex justify-between items-end">
              <div>
                ...
                {isPartial && <div className="text-amber-500 text-xs font-medium uppercase tracking-wider mt-1">Partial Payment</div>}
              </div>
              <div className="text-4xl font-light text-white">RM {parsedAmount.toFixed(2)}</div>
            </div>
          </div>
          {submitError && (
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 sm:space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="space-y-3 sm:space-y-4">
        ...
        {!isLoading && userExpenses.length === 0 && (
          <div className="p-8 text-center border border-zinc-800/50 rounded-2xl bg-zinc-900/50 text-zinc-500">
            You don't have any pending expenses to pay. All your shares are settled!
          </div>
        )}
      </div>
